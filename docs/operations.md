...
`fathom-sim-queue-flow` is centered on simulations work in C#. The technical target is: Create a C# reference implementation for queue workflows, centered on graph analysis, node-edge fixtures, and cycle and reachability reports.
...
The score is deterministic. The current threshold is 179. Risk has a penalty of 5, latency has a penalty of 4, and weight contributes a bonus of 6.
